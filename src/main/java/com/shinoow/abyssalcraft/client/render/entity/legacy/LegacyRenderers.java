...
public final class LegacyRenderers {
    ...
    public static void register(EntityRendererCompat.Renderers renderers, Set<EntityType<?>> handled) {
        ...
        dread(renderers, handled, LegacyEntities.DREADLING.get(),
            ctx -> new LegacyDreadGeoRenderer(ctx, 1.0F, 0.0F, 3.0F, 0.0F, true));
        dread(renderers, handled, LegacyEntities.DREAD_SPAWN.get(),
            ctx -> new LegacyDreadGeoRenderer(ctx, 1.0F, 0.0F, 22.0F, 0.0F, false));
        dread(renderers, handled, LegacyEntities.GREATER_DREAD_SPAWN.get(),
            ctx -> new LegacyDreadGeoRenderer(ctx, 2.0F, 0.0F, 22.0F, 0.0F, false));
        dread(renderers, handled, LegacyEntities.LESSER_DREADBEAST.get(),
            ctx -> new LegacyDreadGeoRenderer(ctx, 3.0F, 0.0F, 22.0F, 0.0F, false));

        shadow(renderers, handled, LegacyEntities.SHADOW_CREATURE.get(), ctx -> new LegacyShadowGeoRenderer(ctx,
            ACRef.id("textures/model/shadowcreature_eyes.png"), "Head1"));
        shadow(renderers, handled, LegacyEntities.SHADOW_MONSTER.get(), ctx -> new LegacyShadowGeoRenderer(ctx,
            ACRef.id("textures/model/shadowmonster_eyes.png"), "Head"));
        shadow(renderers, handled, LegacyEntities.SHADOW_BEAST.get(), ctx -> new LegacyShadowGeoRenderer(ctx,
            ACRef.id("textures/model/elite/shadowbeast_eyes.png"), "head"));
    }
