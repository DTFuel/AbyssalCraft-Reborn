...
import com.shinoow.abyssalcraft.content.block.world.LiquidAntimatterBlock;

/** Cross-loader registration for the Liquid Antimatter world-generation material. */
public final class LiquidAntimatterCompat {

    private static final ResourceLocation STILL = ACRef.id("block/liquid_antimatter_still");
    private static final ResourceLocation FLOWING_TEXTURE = ACRef.id("block/liquid_antimatter_flow");
    ...
    public static final Supplier<FluidType> TYPE = FLUID_TYPES.register("liquid_antimatter",
        LiquidAntimatterCompat::createType);
    public static final Supplier<Fluid> SOURCE = FLUIDS.register("liquid_antimatter", () -> createFluid(true));
    public static final Supplier<Fluid> FLOWING = FLUIDS.register("flowing_liquid_antimatter", () -> createFluid(false));
    public static final Supplier<LiquidBlock> BLOCK = BLOCKS.register("liquid_antimatter", () ->
        new LiquidAntimatterBlock((FlowingFluid) SOURCE.get(), BlockBehaviour.Properties.of().replaceable()
            .noCollission().strength(100.0F).noLootTable().liquid().lightLevel(state -> 8)));
    public static final Supplier<Item> BUCKET = ITEMS.register("liquid_antimatter_bucket", () ->
        new BucketItem(SOURCE.get(), new Item.Properties().craftRemainder(Items.BUCKET).stacksTo(1)));

    private LiquidAntimatterCompat() {}
    ...
    private static FluidType createType() {
        return new FluidType(FluidType.Properties.create()
            .descriptionId("fluid.abyssalcraft.liquid_antimatter")
            .canConvertToSource(true).canSwim(true).canDrown(true)
            .lightLevel(8).density(1000).viscosity(1000)) {
            @Override
