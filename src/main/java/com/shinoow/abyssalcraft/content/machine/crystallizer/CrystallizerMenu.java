...
public final class CrystallizerMenu extends AbstractContainerMenu {

    private static final int MACHINE_SLOTS = 4;
    private final Container machine;
    private final ContainerData data;

    public CrystallizerMenu(int windowId, Inventory inventory, Container machine, ContainerData data) {
        super(Crystallizers.CRYSTALLIZER_MENU.get(), windowId);
        ...
        addSlot(new Slot(machine, MachineBlockEntity.SLOT_FUEL, 56, 53) {
            @Override public boolean mayPlace(ItemStack stack) { return CrystallizerBlockEntity.isFuel(stack); }
        });
        addSlot(new MachineResultSlot(machine, MachineBlockEntity.SLOT_OUTPUT, 116, 35));
        addSlot(new MachineResultSlot(machine, CrystallizerBlockEntity.SLOT_SECONDARY_OUTPUT, 134, 35));
        addPlayerInventory(inventory);
        addDataSlots(data);
    }

    public CrystallizerMenu(int windowId, Inventory inventory, FriendlyByteBuf ignored) {
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        } else if (CrystallizerBlockEntity.isFuel(stack)) {
            if (!moveItemStackTo(stack, 1, 2, false)) return ItemStack.EMPTY;
        } else if (machine instanceof CrystallizerBlockEntity blockEntity && blockEntity.isRecipeInput(stack)) {
