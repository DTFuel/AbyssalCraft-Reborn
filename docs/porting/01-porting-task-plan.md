...
> 并行执行细则见：[平行任务表](02-porting-parallel-tasks.md)。旧版参考（只读）：`docs/AbyssalCraft-1.12.2/`。
...
| ☐ TP.7b | 双加载器试点全流程交互 | M | TP.2b,TP.4b-TP.6b | forge+neoforge 均完成放置→合法燃料→产出→持久化→GUI→JEI 五分类→shift-click | 人工验证矩阵 |
...
| ☐ T1.3c | 建材双端活玩家交互矩阵 | S | T1.3b | Forge/NeoForge 玩家逐项操作门/栅栏门/按钮/压板、观察树叶衰减与原木轴向，记录截图/结果 | 人工客户端矩阵 |
...
| ☐ T1.5c | 装饰行为双端活玩家矩阵 | S | T1.5b | Forge/NeoForge 实际踩 muck/thorn、遮光/传播草块并目视朝向与多面贴图；最终雕像/墓碑高保真几何仍归 T9.2b | 人工客户端矩阵 |
...
| ☐ T1.6c | 工具逐级活玩家挖矿矩阵 | S | T1.6b | Forge/NeoForge 以四级 AC 镐逐级实挖 13 矿，核掉落/速度/错误等级拒绝 | 人工客户端矩阵 |
...
| ☐ T5.2c | Abyssal Wasteland 人工观感/性能矩阵 | M | T5.2b | 双端 spectator 检查高原/湖盆/海岸/枯树与拼缝，并记录新区块生成 p50/p95 | validation/world-visual |
...
| ☐ T5.3c | Dreadlands 人工观感/性能矩阵 | M | T5.3b | 双端 spectator 检查山海过渡、低层洞腔与第五群系拼缝，并记录新区块生成 p50/p95 | validation/world-visual |
...
| ☐ T5.4c | 旧噪声 oracle + 人工观感 | M | T5.4b | 取得 1.12.2 噪声输出做逐位对照；双端目视 Omothol 岛间虚空与 Dark Realm 层叠洞腔 | validation/world-oracle, validation/world-visual |
...
> Gate M5-FIDELITY ☐：旧噪声 oracle、动态 marker 真实玩法、结构/地形人工视觉与最终自然生态矩阵仍待完成。  
> Gate M5-PORTAL ☐：玩家正常制作、激活并往返传送门通过双端验证。
...
| ☐ T6.6e | HUD/字体/reload 人工视觉矩阵 | S | T6.6b-T6.6d | 双端逐项目视且无遮挡/错位 | 人工验证 |

> Gate M6-INFRA ☑：现有 Screen、雾 effects、单粒子、声音资产与 PE meter 注册链可加载。  
> Gate M6-CONTENT ☐：全部 GUI/书页/天空/粒子/声音/HUD 内容完成。  
> Gate M6-UX ☐：两节点视觉、交互与听觉矩阵人工通过。
...
| ☐ T7.1c | 其余消息 handler + 全消息实网矩阵 | L | T7.3b-T7.11b | 每条消息有真实状态变化、线程/权限/方向正确，并在两节点活客户端↔服务器逐条验证 | net/* |
...
| ☐ T7.2c | 活玩家死亡/重连/实网同步矩阵 | M | T7.2b | 双端真实玩家首登 attach、死亡 clone/copyOnDeath、断线重连、停服重启及客户端书副本逐项通过 | system/cap/necrodata/*, net/*, 人工验证 |
...
| ☐ T8.1c | JEI 点击区/配方转移/人工矩阵 | M | T8.1b | 分类、配方、催化剂、点击跳转与容器转移在两节点游戏内通过 | integration/jei/*, 人工验证 |
...
| ID | 任务 | Cx | 依赖 | 验收标准 | 文件 |
|---|---|---|---|---|---|
| ☐ T11.1 | 两节点 `runClient` 全内容冒烟（创造栏/放置/机器/GUI/实体/维度/仪式/法术/PE） | L | M10 | 冒烟清单逐项通过、无崩溃 | 验证 |
| ☐ T11.2 | `runServer` 世界生成/结构/刷怪/战利品验证矩阵 | M | M10 | 4 维度+结构+刷怪+掉落全绿 | 验证 |
| ☐ T11.3 | 回写 `DEVELOPMENT.md` §7/8/9 + 更新 `docs/index` | S | T11.1,T11.2 | 文档反映真实状态 | DEVELOPMENT.md, docs/index/* |
| ☐ T11.4 | 发布产物核对（jar 元数据/依赖/许可/版本） | S | T11.1 | jar 内元数据/许可/版本正确 | build 产物 |

> Gate M11 = Definition of Done（见下）。
