...
- 状态：旧27项中22项MIGRATED、5项BLOCKED，statue/depositioner触发链完成
- 负责：GitHub Copilot
- 最后更新：2026-07-25

## 1. 概述 / 目标

AbyssalCraft 的势能扰动系统。statue/depositioner在PoP外传输累积容差，达到100/200后由`DisruptionHandler`按deity过滤并执行随机扰动。当前忠实迁移了所有依赖已满足的22项，并以机器可读审计保留5项BLOCKED，不用替代内容伪造完整27项。

## 2. 范围

- MIGRATED 22项：lightning、fire、spawnShoggoth、5 potion、potentialEnergy、freeze、swarmShadow、fireRain、displace、randomTeleport、potentialEnergyDrain、swarmSheep、animalCorruption、spawnShubOffspring、两sacrificeCorruption、两famine。
- Animal corruption覆盖旧明确分支：牛/鸡/猪/羊→Evil，普通马→亡灵马，驯服狼/猫解除驯服，兔→杀手兔。
- BLOCKED 5项：`ooze`（缺shoggoth_ooze块）、`randomSwarm/randomSpawn`（候选生态未冻结）、两`invisibleSwarm`（忠实隐形群体设计未落地）。详见`rr-energy-disruption-audit.csv`。
...
- 关键类：
  ...
  - `Disruptions`（内容注册，`bootstrap()` 主类 `init` 挂）：注册22项MIGRATED扰动，覆盖spawn/swarm、药水、玩家、PE、腐化、位移和火焰族；集合及deity映射由`DisruptionAudit`对照`rr-energy-disruption-audit.csv`永久校验。
...
- **框架先于内容（历史实施路径）**：PS-9先交付抽象基、handler和fork-free pilot；RR-ENERGY随后在实体/能量依赖就绪后扩至22项MIGRATED，并将仍不满足忠实实现条件的5项固定为BLOCKED。
...
- 永久`DisruptionAudit`精确断言注册集合=22、BLOCKED=5、总闭包27、deity映射无漂移；双端`RR_ENERGY_SELF_TEST_OK ... disruptions=22 blocked=5`。
