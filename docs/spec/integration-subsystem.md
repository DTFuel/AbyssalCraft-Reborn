...
- **PJ-4b**：IMC handler（`InterModProcessEvent`）+ IACPlugin 插件发现 API——recipe-添加类 IMC key 在数据包配方下已过时；shoggothFood 需未移植 shoggoth AI 食物注册；IACPlugin 无消费者。
...
**进度**：纯数据包 JSON。1.12.2 单 item 对象 `[{"item":"id"}]` → 现代 items 列表 `{"items":["id"]}`；display/title/description/parent/background/requirements 保留。dual-dir（复数/单数）与 recipes 同法：每加载器只读其目录。
...
- 进度 id 沿用 1.12.2（root/mine_*/ethaxium/dreadium/shadow_gems），item id 用现代注册名（`coralium_ore` 等）；necronomicon 未移植 → root 用 `dreadium_ingot` 替代（占位，物品移植后还原 = PK/内容）。
...
- **进度 fork**：仅目录名 `advancements/`（1.20.1）↔ `advancement/`（1.21）；JSON `{"items":["id"]}` ItemPredicate 两端同解析。
...
- 2026-07-25：RR-KNOWLEDGE（CR-70）完成PJ-2b非标量定义/解析/reload与necrodata客户端同步；PJ-2c全消费者+GUI、T8.3b真人命令矩阵仍待办。
